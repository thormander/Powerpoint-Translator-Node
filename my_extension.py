...
class TemplateNode:
    ...
    # Will probably remove this as it isn't the right one as you can't pick directories
    ...
    def execute(self, ctx: knext.ExecutionContext):
        ...
        # Fetch parameters from flow variables if they are passed in, otherwise just use default
        ...
        # Resolve the file path for KNIME compatibility
        self.input_file_path = self.resolve_file_path(self.input_file_path)

        # Process the translation using the fetched or default parameters
        ...
        # Here we want to output the file location as a variable so we modify output_file variable and replace it with translated file path
    ...
    def create_output_file(self, filename):
        ...
        """
        # Create the output filename by appending the target language
        output_filename = f"{self.target_language}_translated_{self.output_name}.pptx"
        
        # Determine the output directory (either user-specified or default to Desktop)
        if self.output_file:  # Use the user-specified output directory if provided
            ...
            # Hardcoded path to the desktop as fallback
            output_path = os.path.join("/Users/____/Desktop", output_filename)
        
        return output_path


    # Utility function to resolve file path (KNIME URI to system path)
    ...
    def translate_text(self, text):
        ...
        system_message = f"You are a translator for PowerPoint presentations. Translate to {self.target_language}, keeping proper nouns unchanged."
        ...
        LOGGER.info(f"API Response: {response.json()}")  # Log the full response for debugging
